...
export default class Target extends Laya.Script3D {
    ...
    setType(type: number) {
        ...
        if (this.type === Const.TargetType.DEFAULT) {
            ...
            this.material.albedoColor = new Laya.Vector4(1, 1, 1, 0.7);
            this.material.specularColor = new Laya.Vector4(0, 0, 0, 1);
            this.material.enableEmission = true;
    ...
    private effectPiecesBroken() {
        Laya.Mesh.load(Const.PieceResUrl, Laya.Handler.create(this, (mesh) => {
            ...
            for (let i = 0; i < pieceNum; i++) {
                ...
                mat.albedoColor = new Laya.Vector4(1, 1, 1, 0.7);
                ...
                Laya.Texture2D.load(Const.StageTexUrl[2], Laya.Handler.create(this, (tex) => {
                    (piece.meshRenderer.material as Laya.PBRSpecularMaterial).albedoTexture = tex;
                }));
                ...
                // add pieces to scene
                GameScene.instance.scene3D.addChild(piece);
                GameScene.instance.piecesList.push(piece);
                ...
                piece.transform.localScale = new Laya.Vector3(scaleX, scaleY, scaleZ);
                // set rot
                piece.transform.localRotationEulerX = (Math.random() - 0.5) * 2 * 90;
                piece.transform.localRotationEulerY = (Math.random() - 0.5) * 2 * 90;
                piece.transform.localRotationEulerZ = (Math.random() - 0.5) * 2 * 90;
                // set pos
                let velocity: Laya.Vector3 = new Laya.Vector3();
                ...
                piece.transform.position = this.target.transform.position.clone();
                Laya.Vector3.add(piece.transform.localPosition, velocity, piece.transform.localPosition);
                ...
                (piece.getComponent(Laya.Rigidbody3D) as Laya.Rigidbody3D).linearVelocity = velocity.clone();
                // set hiding effect
                Laya.Tween.to(piece.transform.localScale, { x: scaleX / 2, y: scaleY / 2, z: scaleZ / 2 }, Const.PiecesBrokenTime / 60 * 1000, Laya.Ease.linearNone);
                Laya.Tween.to(piece.meshRenderer.material, { albedoColorA: 0 }, Const.PiecesBrokenTime / 60 * 1000, Laya.Ease.linearNone, Laya.Handler.create(this, () => {
                    piece.destroy();
                }));
            }
        }));
