...
export default class GameScene extends ui.game.GameSceneUI {
    ...
    onOpened(param?: any) {
        ...
        if (this.state === Const.GameState.START) {
            ...
            // change bg
            if (Global.gameData.stageIndex <= 2 && this.bgIdx != 0) {
                this.bgIdx = 0;
                this.newBackground();
            }
            else if (Global.gameData.stageIndex > 2 && this.bgIdx != (Global.gameData.stageIndex - 2) % 4) {
    ...
    public rewardCannonLeft: number = -1;
    ...
    initBullet() {
        ...
        // this._bullet = new Laya.MeshSprite3D(Laya.PrimitiveMesh.createSphere(0.08));
    }
    ...
    newTrail() {
        ...
        else if (Global.gameData.trailType == 3) {
    ...
    /** new background */
    newBackground() {
        // init invisble ground
        if (!this.ground) {
            this.ground = new Laya.MeshSprite3D();
            ...
            planeCollider.colliderShape = new Laya.StaticPlaneColliderShape(new Laya.Vector3(0, 1, 0), -2.7688);
        }
        // destroy old
        if (this.background) {
    ...
    private bindButtons() {
        ...
        this.onClick_rewardCannon = () => {
            ...
            this.rewardCannonLeft = 2;
            this.label_rewardCannon.changeText("剩余" + this.rewardCannonLeft + "小关");
            this.label_rewardCannon.visible = true;
        ...
        this.btn_backExit.on(Laya.Event.CLICK, this, () => {
            ...
            if (this.isRewardCannon) {
                this.setCannon(Global.gameData.cannonType);
                this.isRewardCannon = false;
                this.rewardCannonLeft = -1;
                this.label_rewardCannon.visible = false;
            }
            // clear timer
    ...
    newStage() {
        ...
        // update reward cannon
        this.label_rewardCannon.visible = false;
        if (this.missionIdx >= 1 && this.missionIdx <= 5 && this.isRewardCannon) {
            this.rewardCannonLeft--;
            if (this.rewardCannonLeft < 0) {
                this.setCannon(Global.gameData.cannonType);
                this.isRewardCannon = false;
                this.rewardCannonLeft = -1;
                this.label_rewardCannon.visible = false;
        ...
        Laya.Sprite3D.load(satgeResUrl, Laya.Handler.create(this, (res) => {
            ...
            Laya.timer.frameOnce(15, this, () => {  // todo: for test, 15 chang to 60
                ...
                if (this.stageIdx === 1) {
                    ...
                    else if (this.missionIdx === 3 && Global.gameData.tutorialStep === 2) {
    ...
    private winCheck() {
        ...
        if (this.winCheckCnt++ >= Const.MaxWinCheckTime) {
            ...
            if (Laya.Browser.onMiniGame && Global.gameData.soundEnabled) {
                Laya.SoundManager.playSound(Const.soundUrl[2]);
            }
    ...
    bulletBombEff(pos: Laya.Vector3) {
        ...
        mat.albedoColor = new Laya.Vector4(1, 0.7, 0.3, 0.3);
        // n帧后销毁隐形炸弹
        let cnt: number = 0;
        bomb.timer.frameLoop(1, bomb, () => {
            ...
            bomb.transform.localScaleX += 0.1;
            bomb.transform.localScaleY += 0.1;
            bomb.transform.localScaleZ += 0.1;
            let tmpMat = bomb.meshRenderer.material as Laya.UnlitMaterial;
            tmpMat.albedoColorA > 0.1 && (tmpMat.albedoColorA -= 0.1);
        });
    }
    ...
    private createBullet() {
        ...
        if (Laya.Browser.onMiniGame && Global.gameData.soundEnabled) {
            if (!this.isRewardBullet) {
                ...
                Laya.SoundManager.playSound(Const.soundUrl[1]);
            }
        }
        // vibration
